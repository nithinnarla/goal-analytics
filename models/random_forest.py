...
international results dataset (data.historical.fetch_results()), giving
it far more training signal than the ~900-match World-Cup-only logistic
model while staying focused on results that reflect the modern game.
...
Complements the existing models:
  ...
  - RandomForest:       non-linear bagged-tree ensemble, trained on
                         recent-era international results — used as an
...
-----
    ...
    model = RandomForestMatchPredictor()
...
try:
    ...
    warnings.warn(
        "scikit-learn not installed. RandomForestMatchPredictor will use Elo fallback.\n"
...
class RandomForestMatchPredictor:
    """
    Random-forest classifier over the same 6-feature vector as
    LogisticMatchPredictor, intended to be trained on the recent-era
    (2010+) slice of international results rather than just WC matches.
    """
    ...
    def train(self, X, y, cv: int = 5) -> "RandomForestMatchPredictor":
        ...
        if len(np.unique(y)) == 3:
            ...
            print(f"  RandomForest trained on {self.n_train} matches | {cv}-fold CV accuracy: {self.cv_accuracy:.1%}")
        return self

    @classmethod
    def train_from_history(cls, wc_df, elos: dict, form_lookup: dict, **kwargs) -> "RandomForestMatchPredictor":
        model = cls(**kwargs)
